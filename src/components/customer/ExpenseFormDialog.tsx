...
export interface ExpenseFormData {
  amount: number; expense_date: string; type: string; category: string; status: string; payment_method: string; description: string; num_payments: number;
}

const empty: ExpenseFormData = { amount: 0, expense_date: new Date().toISOString().split("T")[0], type: "חודשי", category: "", status: "מאושר", payment_method: "ללא", description: "", num_payments: 1 };

interface Props {
  open: boolean; onOpenChange: (open: boolean) => void;
  initialData?: (ExpenseFormData & { id?: string }) | null;
  onSave: (data: ExpenseFormData, id?: string) => void;
}

export function ExpenseFormDialog({ open, onOpenChange, initialData, onSave }: Props) {
  ...
  const set = (key: keyof ExpenseFormData, value: any) => setForm((p) => ({ ...p, [key]: value }));
  ...
  const handleCreateCategory = async () => {
    ...
    if (!name) return;
    await createCategory({ name, kind: "expense" } as any);
    set("category", name);
    setAddingCategory(false);
    setNewCategory("");
  };
      ...
      <DialogContent className="sm:max-w-md rounded-2xl border-border/50 shadow-2xl" dir="rtl">
        <DialogHeader>
          <DialogTitle className="text-xl font-extrabold flex items-center gap-2">
            <div className="h-9 w-9 rounded-xl gradient-primary flex items-center justify-center shadow-glow-sm"><TrendingDown className="h-4 w-4 text-primary-foreground" /></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2"><Label className="font-semibold text-xs">תאריך</Label><Input type="date" value={form.expense_date} onChange={(e) => set("expense_date", e.target.value)} className="rounded-xl" dir="ltr" /></div>
